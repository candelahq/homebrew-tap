# typed: false
# frozen_string_literal: true

class Candela < Formula
  desc "LLM observability proxy — traces, costs, budgets for AI dev tools"
  homepage "https://github.com/candelahq/candela"
  license "Apache-2.0"

  on_macos do
    on_arm do
      url "https://github.com/candelahq/candela/releases/download/v0.11.1/candela_0.11.1_Darwin_universal.tar.gz"
      sha256 "666f445f2089cc9e1e2e625ed6f1916d0d685338e809def6eacca8a525b13ab2"
    end
    on_intel do
      url "https://github.com/candelahq/candela/releases/download/v0.11.1/candela_0.11.1_Darwin_universal.tar.gz"
      sha256 "666f445f2089cc9e1e2e625ed6f1916d0d685338e809def6eacca8a525b13ab2"
    end
  end

  on_linux do
    on_intel do
      url "https://github.com/candelahq/candela/releases/download/v0.11.1/candela_0.11.1_Linux_amd64.tar.gz"
      sha256 "1b1b86883d67c713a0dc21a37b26c8cde8d5b16dbe3b06ad2f87783ea38e24af"
    end
    on_arm do
      url "https://github.com/candelahq/candela/releases/download/v0.11.1/candela_0.11.1_Linux_arm64.tar.gz"
      sha256 "62c4cb473b0facf3ed86f81b1d51a0e5e042d32d4fd0daa39e0bf37bfa1d7ce9"
    end
  end

  def install
    bin.install "candela"
  end

  def caveats
    <<~EOS
      To get started, create a config file:

        mkdir -p ~/.config/candela
        cat > ~/.config/candela/config.yaml <<EOF
        port: 8181
        providers:
- name: google
  models: ["gemini-2.5-pro", "gemini-2.5-flash", "gemini-3.5-flash"]
- name: anthropic
  models: ["claude-sonnet-4-20250514"]
        vertex_ai:
project: YOUR_GCP_PROJECT
region: us-central1
        EOF

      Authenticate (no gcloud required):
        candela auth login             # GCP OAuth2 (default)
        candela auth login --provider aws  # AWS SSO/access keys

      Then run:
        candela start          # background daemon
        candela status         # check running state
        candela run            # foreground (debug)

      Management UI: http://127.0.0.1:8181/_local/

      Or as a login service:
        brew services start candela
    EOS
  end

  service do
    run [opt_bin/"candela", "run"]
    keep_alive true
    log_path var/"log/candela.log"
    error_log_path var/"log/candela.log"
  end

  test do
    system "#{bin}/candela", "version"
  end
end
