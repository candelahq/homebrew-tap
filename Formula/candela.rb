# typed: false
# frozen_string_literal: true

class Candela < Formula
  desc "LLM observability proxy — traces, costs, budgets for AI dev tools"
  homepage "https://github.com/candelahq/candela"
  license "Apache-2.0"

  on_macos do
    on_arm do
      url "https://github.com/candelahq/candela/releases/download/v0.11.4/candela_0.11.4_Darwin_universal.tar.gz"
      sha256 "d565ade21ab4640a7d95eb325c02cb17e52811d764aae8c969b8b86b3b802429"
    end
    on_intel do
      url "https://github.com/candelahq/candela/releases/download/v0.11.4/candela_0.11.4_Darwin_universal.tar.gz"
      sha256 "d565ade21ab4640a7d95eb325c02cb17e52811d764aae8c969b8b86b3b802429"
    end
  end

  on_linux do
    on_intel do
      url "https://github.com/candelahq/candela/releases/download/v0.11.4/candela_0.11.4_Linux_amd64.tar.gz"
      sha256 "837fe822c9625f795a26fb9eb19e900a1a3e2e5ebb34b31c3d1f0d2f4bed5fd3"
    end
    on_arm do
      url "https://github.com/candelahq/candela/releases/download/v0.11.4/candela_0.11.4_Linux_arm64.tar.gz"
      sha256 "94cadc812a85a2591768d3057e6843aad8930742c2b6d9ee9df1d02b7ab0d9e6"
    end
  end

  def install
    bin.install "candela"
  end

  def caveats
    <<~EOS
      To get started, create a config file:

        mkdir -p ~/.config/candela
        cat > ~/.config/candela/config.yaml <<EOF
        port: 8181
        providers:
          - name: google
            models: ["gemini-2.5-pro", "gemini-2.5-flash", "gemini-3.5-flash"]
          - name: anthropic
            models: ["claude-sonnet-4-20250514"]
        vertex_ai:
          project: YOUR_GCP_PROJECT
          region: us-central1
        EOF

      Authenticate (no gcloud required):
        candela auth login             # GCP OAuth2 (default)
        candela auth login --provider aws  # AWS SSO/access keys

      Then run:
        candela start          # background daemon
        candela status         # check running state
        candela run            # foreground (debug)

      Management UI: http://127.0.0.1:8181/_local/

      Or as a login service:
        brew services start candela
    EOS
  end

  service do
    run [opt_bin/"candela", "run"]
    keep_alive true
    log_path var/"log/candela.log"
    error_log_path var/"log/candela.log"
  end

  test do
    system "#{bin}/candela", "version"
  end
end
