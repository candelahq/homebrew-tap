# typed: false
# frozen_string_literal: true

class Candela < Formula
  desc "LLM observability proxy — traces, costs, budgets for AI dev tools"
  homepage "https://github.com/candelahq/candela"
  license "Apache-2.0"

  on_macos do
    on_arm do
      url "https://github.com/candelahq/candela/releases/download/v0.9.3/candela_0.9.3_Darwin_universal.tar.gz"
      sha256 "9009733abbbdb12d677657146ae149e1eed02086a8f6435fda60477158dd4da9"
    end
    on_intel do
      url "https://github.com/candelahq/candela/releases/download/v0.9.3/candela_0.9.3_Darwin_universal.tar.gz"
      sha256 "9009733abbbdb12d677657146ae149e1eed02086a8f6435fda60477158dd4da9"
    end
  end

  on_linux do
    on_intel do
      url "https://github.com/candelahq/candela/releases/download/v0.9.3/candela_0.9.3_Linux_amd64.tar.gz"
      sha256 "911361fc116c7057b7e775efe7d58d59f41ac378aba8c14d4133eb01392cca25"
    end
    on_arm do
      url "https://github.com/candelahq/candela/releases/download/v0.9.3/candela_0.9.3_Linux_arm64.tar.gz"
      sha256 "2c2a7f7091d9bbc982a32492fc33ce3aa11ad446dc0ff06bd24d4d59c1074071"
    end
  end

  def install
    bin.install "candela"
  end

  def caveats
    <<~EOS
      To get started, create a config file:

        mkdir -p ~/.config/candela
        cat > ~/.config/candela/config.yaml <<EOF
        port: 8181
        providers:
- name: google
  models: ["gemini-2.5-pro", "gemini-2.5-flash", "gemini-3.5-flash"]
- name: anthropic
  models: ["claude-sonnet-4-20250514"]
        vertex_ai:
project: YOUR_GCP_PROJECT
region: us-central1
        EOF

      Authenticate (no gcloud required):
        candela auth login             # GCP OAuth2 (default)
        candela auth login --provider aws  # AWS SSO/access keys

      Then run:
        candela start          # background daemon
        candela status         # check running state
        candela run            # foreground (debug)

      Management UI: http://127.0.0.1:8181/_local/

      Or as a login service:
        brew services start candela
    EOS
  end

  service do
    run [opt_bin/"candela", "run"]
    keep_alive true
    log_path var/"log/candela.log"
    error_log_path var/"log/candela.log"
  end

  test do
    system "#{bin}/candela", "version"
  end
end
